            ...
            if (rule[Config.FIELD_meThodName] == node.methodName && isInResources) {
                ...
                if(param.matches(reg)){
                    ...
                    if(rule[Config.FIELD_needFix].trim() != "1"){
                        context.report(createMethodParamIssue(rule, this), node, context.getLocation(node), rule[Config.FIELD_ID])
                    }
                    else{
                        val oldText = getParamTextFromMethod(node, rule[Config.FIELD_paramIndex].toInt())
                        context.report(createMethodParamIssue(rule, this), node, context.getLocation(node), rule[Config.FIELD_ID], createMethodParamIssueQuickFix(context, node, rule, oldText))
                    }
                }
            }
        }
