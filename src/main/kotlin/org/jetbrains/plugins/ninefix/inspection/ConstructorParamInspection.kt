            ...
            if (rule[Config.FIELD_className].split('.').last() == constructor.name){
                ...
                if (param.matches(reg)) {
                    if(rule[Config.FIELD_needFix].trim() != "1"){
                        context.report(createConstructureParamIssue(rule, this), node, context.getLocation(node), rule[Config.FIELD_ID])
                    }
                    else{
                        val oldText = getParamTextFromConstructor(node, rule[Config.FIELD_paramIndex].toInt())
                        context.report(createConstructureParamIssue(rule, this), node, context.getLocation(node), rule[Config.FIELD_ID], createConstructorParamIssueQuickFix(context, node, rule, oldText))
                    }
                }
            }
        }
