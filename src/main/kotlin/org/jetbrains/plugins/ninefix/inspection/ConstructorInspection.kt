        ...
        for (rule in rules){
            ...
            if (rule[Config.FIELD_className].split('.').last() == constructor.name){
                if(rule[Config.FIELD_needFix].trim() != "1"){
                    context.report(createConstructorIssue(rule, this), node, context.getLocation(node), rule[Config.FIELD_briefDescription])
                }
                else{
                    val oldText = getVariableNameFromDeclare(node)
                    context.report(createConstructorIssue(rule, this), node, context.getLocation(node), rule[Config.FIELD_briefDescription], createConstructorIssueQuickFix(context, node, rule, oldText))
                }
            }
        }
    }
