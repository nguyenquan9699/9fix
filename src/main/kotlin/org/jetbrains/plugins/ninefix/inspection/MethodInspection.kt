        ...
        for (rule in rules){
            ...
            if (rule[Config.FIELD_meThodName] == node.methodName && isInResources) {
                if(rule[Config.FIELD_needFix].trim() != "1"){
                    context.report(createMethodIssue(rule, this), node, context.getLocation(node), rule[Config.FIELD_ID])
                }
                else{
                    context.report(createMethodIssue(rule, this), node, context.getLocation(node), rule[Config.FIELD_ID], createMethodIssueQuickFix(context, node, rule))
                }
            }
        }
    }
